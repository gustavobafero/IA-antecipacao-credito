...
if enviar:
    ...
    # Novo cálculo da taxa ideal com possibilidade de ficar abaixo da concorrência
    fator_risco = risco * 1.5  # peso ajustável do risco
    taxa_ideal = round(custo_capital + margem_desejada + fator_risco, 2)

    margem_estimada = round(taxa_ideal - custo_capital, 2)
    retorno_esperado = round((taxa_ideal - custo_capital) / 100 * valor, 2)

    if taxa_ideal > taxa_concorrencia:
        status = "Acima do mercado"
    elif taxa_ideal < taxa_concorrencia:
